...
VE_TextureT *VE_Render_LoadTexture(const char *pTexturePath, VE_LoadTextureOptionsT *pOptions) {
    ...
    vkCreateImage(VE_G_Device, &imageInfo, NULL, &pTexture->image);

    VkMemoryRequirements memRequirements;
    vkGetImageMemoryRequirements(VE_G_Device, pTexture->image, &memRequirements);
    ...
    vkAllocateMemory(VE_G_Device, &allocInfo, NULL, &pTexture->memory);

    vkBindImageMemory(VE_G_Device, pTexture->image, pTexture->memory, 0);

    VE_Render_TransitionImageLayout(pTexture->image, VK_FORMAT_R8G8B8A8_SRGB, VK_IMAGE_LAYOUT_UNDEFINED, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL);
    VE_Render_CopyBufferToImage(stagingBuffer, pTexture->image, (uint32_t) texWidth, (uint32_t) texHeight);
    VE_Render_TransitionImageLayout(pTexture->image, VK_FORMAT_R8G8B8A8_SRGB, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL);
    ...
    vkCreateImageView(VE_G_Device, &viewInfo, NULL, &pTexture->imageView);
